...
_DEFAULT_BASE = "/kaggle/working/frbot" if Path("/kaggle/working").exists() else "/tmp/frbot"
BASE  = Path(os.getenv("FRBOT_BASE_DIR", _DEFAULT_BASE))
FILES = BASE / "files"
TMP   = BASE / "tmp"
WORK  = BASE / "work"
DL    = BASE / "downloads"
...
ENCODE_CONCURRENCY = max(1, int(os.getenv("ENCODE_CONCURRENCY", "3")))
_encode_sem = asyncio.Semaphore(ENCODE_CONCURRENCY)
    ...
    for cue in cues:
        ...
        for active in active_subs:
            ...
            if is_sim:
                ...
                a_bg = active.get("bg", (0,0,0)); c_bg = cue.get("bg", (0,0,0))
                active["bg"] = tuple(int((a+c)//2) for a,c in zip(a_bg, c_bg))
                matched = True
                break
...
def _split_by_spatial_proximity(cluster: list, frame_w: int, threshold: float = 0.30) -> list:
    """Split a time-cluster into spatially separate groups by x-distance."""
    if len(cluster) <= 1:
        return [cluster]
    sorted_c = sorted(cluster, key=lambda c: float(c.get("x", frame_w / 2)))
    sub_clusters, cur = [], [sorted_c[0]]
    for item in sorted_c[1:]:
        if abs(float(item.get("x", frame_w/2)) - float(cur[-1].get("x", frame_w/2))) <= threshold * frame_w:
            cur.append(item)
        else:
...
def consolidate_overlay_clusters(cues: list, frame_w: int, frame_h: int) -> list:
    ...
    merged_upper = []
    for time_cluster in _cluster_by_time_overlap(upper):
        # Split each time-cluster by spatial proximity before merging
        for cluster in _split_by_spatial_proximity(time_cluster, frame_w):
            if len(cluster) == 1:
            ...
            bws = [float(c.get("bw", 0)) for c in cluster]
            bgs = [c.get("bg", (0,0,0)) for c in cluster]
            cluster_x  = sum(xs) / len(xs)
            cluster_y  = sum(ys) / len(ys)
            cluster_bh = sum(bhs) / len(bhs) if any(bhs) else 0.0
            cluster_bw = sum(bws) / len(bws) if any(bws) else 0.0
            cluster_bg = tuple(int(sum(ch[i] for ch in bgs) / len(bgs)) for i in range(3))
...
# ══════════════════════════════════════════════════════════════════════════════
def _sample_bg_color(frame: np.ndarray, xs: list, ys: list) -> tuple:
    """Return median BGR of the background region around a text bounding box."""
    h, w = frame.shape[:2]
    x0, x1 = int(min(xs)), int(max(xs))
    y0, y1 = int(min(ys)), int(max(ys))
    pad = max(int((y1 - y0) * 0.3), 4)
   
    regions = []
    if y0 - pad >= 0:
        regions.append(frame[max(y0-pad, 0):y0,   max(x0,0):min(x1,w)])
    if y1 + pad <= h:
        regions.append(frame[y1:min(y1+pad, h),   max(x0,0):min(x1,w)])
    if not regions:
        regions.append(frame[max(y0,0):min(y1,h), max(x0,0):min(x1,w)])
    combined = np.vstack([r for r in regions if r.size > 0])
    if combined.size == 0:
        return (0, 0, 0)
    median = np.median(combined.reshape(-1, 3), axis=0)
...
def run_ocr_pipeline(video_path: str, status_msg, chat_id: int,
    ...
    NUM_WORKERS = max(1, int(os.getenv("OCR_WORKERS", "8")))
...
def _split_simple(text: str, is_cjk: bool):
    """Split text into at most 2 lines at roughly the midpoint."""
    if not text:
        return []
    if is_cjk:
        if len(text) <= 6:
...
def _derive_card_colors(bg_bgr: tuple) -> tuple:
    """Derive text/outline/shadow ASS colour strings from a sampled background BGR."""
    b, g, r = bg_bgr
    luminance = 0.299 * r + 0.587 * g + 0.114 * b

    if luminance < 128:
        # Dark background — light text with a subtle warm tint
        tint_r = min(255, 210 + int((r - 128) * 0.15))
        tint_g = min(255, 200 + int((g - 128) * 0.10))
        tint_b = min(255, 195 + int((b - 128) * 0.10))
        text_r, text_g, text_b = tint_r, tint_g, tint_b
        # Outline: bg colour shifted slightly darker/warmer
        out_r = max(0, int(r * 0.35))
        out_g = max(0, int(g * 0.30))
        out_b = max(0, int(b * 0.28))
    else:
        # Light background — dark text
        tint_r = max(0, 30 + int((r - 128) * 0.10))
        tint_g = max(0, 20 + int((g - 128) * 0.10))
        tint_b = max(0, 15 + int((b - 128) * 0.08))
        text_r, text_g, text_b = tint_r, tint_g, tint_b
        out_r = min(255, int(r * 1.3))
        out_g = min(255, int(g * 1.3))
        out_b = min(255, int(b * 1.3))

    shad_r = max(0, out_r - 30)
    shad_g = max(0, out_g - 25)
    shad_b = max(0, out_b - 25)

    def to_ass(rv, gv, bv):
        return f"&H00{bv:02X}{gv:02X}{rv:02X}"

    return to_ass(text_r, text_g, text_b), to_ass(out_r, out_g, out_b), to_ass(shad_r, shad_g, shad_b)
...
Style: Sign,Georgia,{FONT_SIZE},&H00D4CEC5,&H000000FF,&H00312C25,&H801D2320,-1,0,0,0,85,100,0,0,1,1,1,2,20,20,20,1
...
"""
    ...
    for sub, en in zip(subs, en_texts):
        ...
        # CARD / OVERLAY — smart side placement.
       
       
       
       
        # cards it naturally falls to above/below.
        if sub.get("_cluster"):
        ...
        bw = float(sub.get("bw", 0)) * scale_x   # card box width in PlayRes units
        if bw == 0:
            bw = bh * 0.6   # fallback estimate if bw wasn't stored

        # Card bounding box edges in PlayRes space
        card_left   = x - bw / 2
        card_right  = x + bw / 2
        card_top    = y - bh / 2
        card_bottom = y + bh / 2

        GAP = SAFE_MARGIN * 1.5   # breathing room between card edge and subtitle

        sp_left   = card_left  - SAFE_MARGIN
        sp_right  = play_x - card_right - SAFE_MARGIN
        sp_above  = card_top   - SAFE_MARGIN
        sp_below  = play_y - card_bottom - SAFE_MARGIN

       
       
       
       
        original_char_count = sum(len(item) for item in zh_items) if zh_items else 1
        card_char_h   = (bh / max(original_char_count, 1)) if bh > 0 else 36
        base_fs       = max(min(round(card_char_h), FONT_SIZE), 30)
        base_trans_fs = max(round(base_fs * 0.85), 24)

        def _est_text_w(f, tf):
            lz = max((len(l) for l in zh_lines), default=0)
            le = max((len(l) for l in en_lines), default=0) if en_lines else 0
            return max(lz * f * 0.95, le * tf * 0.50)

        def _est_text_h(f, tf):
            zh_h = len(zh_lines) * f * 1.3
            en_h = len(en_lines) * tf * 1.3 if en_lines else 0
            return zh_h + en_h

        def _fit_in(avail_w, avail_h):
            """Shrink from base_fs until text fits avail_w x avail_h, or return None."""
            f, tf = base_fs, base_trans_fs
            for _ in range(8):
                if _est_text_w(f, tf) <= avail_w and _est_text_h(f, tf) <= avail_h:
                    return f, tf
                if f <= 22:
                    return None
                f  = max(round(f  * 0.85), 22)
                tf = max(round(f  * 0.75), 18)
            return None

        # Try each side in order of available space (largest first).
       
       
       
        # the vertical space available.
        sides = sorted([
            ("left",  sp_left,  sp_left,  bh),
            ("right", sp_right, sp_right, bh),
            ("above", sp_above, play_x - 2 * SAFE_MARGIN, sp_above),
            ("below", sp_below, play_x - 2 * SAFE_MARGIN, sp_below),
        ], key=lambda s: -s[1])   # largest available first

        chosen = None
        for side_name, _, avail_w, avail_h in sides:
            result = _fit_in(max(avail_w - GAP, 0), avail_h)
            if result:
                chosen = (side_name, result[0], result[1])
                break
        ...
        if not chosen:
            ...
            if en_clean:
                ...
                text = f"[{zh_block}]"
            events.append(f"Dialogue: 0,{ts_s},{ts_e},Sign,SIGN,0,0,0,,{text}")
            continue

        side_name, fs, trans_fs = chosen
        zh_ov = f"\\fs{fs}" if fs != FONT_SIZE else ""
        en_ov = f"\\fs{trans_fs}" if trans_fs != FONT_SIZE else ""

        bg_bgr = sub.get("bg", (20, 20, 20))
        text_col, out_col, shad_col = _derive_card_colors(bg_bgr)
        card_base = (f"\\fad(400,0)\\fnGeorgia\\b1\\fscx85\\bord1\\shad1"
                     f"\\c{text_col}\\3c{out_col}\\4c{shad_col}")

        y_mid = (card_top + card_bottom) / 2.0   # vertical center of the card

        if side_name == "left":
            # Anchor at right edge of available left space, right-aligned,
            # vertically centered alongside the card (\an6 = middle-right)
            pos_x = card_left - GAP
            pos_y = y_mid
            an_tag = "6"
        elif side_name == "right":
            # Anchor at left edge of available right space, left-aligned,
            # vertically centered alongside the card (\an4 = middle-left)
            pos_x = card_right + GAP
            pos_y = y_mid
            an_tag = "4"
        elif side_name == "above":
            pos_x = x
            pos_y = card_top - GAP
            an_tag = "2"   # bottom-center, grows upward
        else:  # below
            pos_x = x
            pos_y = card_bottom + GAP
            an_tag = "8"   # top-center, grows downward

        if en_clean:
            if an_tag in ("4", "8"):   # left-side or below: zh first then en
                text = (f"{{\\an{an_tag}\\pos({pos_x:.0f},{pos_y:.0f}){card_base}{zh_ov}}}{zh_block}"
                        f"\\N{{{en_ov}}}{en_block}")
            else:   # right-side or above: en first then zh
                text = (f"{{\\an{an_tag}\\pos({pos_x:.0f},{pos_y:.0f}){card_base}{en_ov}}}{en_block}"
                        f"\\N{{{zh_ov}}}{zh_block}")
        else:
            text = f"{{\\an{an_tag}\\pos({pos_x:.0f},{pos_y:.0f}){card_base}{zh_ov}}}{zh_block}"
...
def build_encode_cmd(input_path: Path, out_path: Path, spec: QualitySpec,
                     ...
                     audio_idx: str = "a:0", has_subs: bool = True,
                     gpu_id: int = 0) -> list:
...
async def run_encode(task: Task, spec: QualitySpec, out_path: Path, prog_msg: Message):
    ...
    async with _encode_sem:   
        ...
        quality_index = next(
            (i for i, item in enumerate(QUALITY_SPECS) if item.label == spec.label), 0
        )
        gpu_id = quality_index % max(NUM_GPUS, 1)
        cmd = build_encode_cmd(
            task.input_path, out_path, spec, task.output_name,
            task.src_bitrate, audio_idx, has_subs, gpu_id=gpu_id
        )
        log.info(f"[{spec.label}] hevc_nvenc p7+multipass start on GPU:{gpu_id}")
...
# ── Upload ────────────────────────────────────────────────────────────────────
TG_LIMIT  = 2 * 1024 * 1024 * 1024   # 2 GB Telegram bot upload limit
LEECH_URL = os.getenv("LEECH_URL", "").rstrip("/")   # e.g. https://your-leech-server.com — set in env to enable
...
async def upload_file(chat_id: int, path: Path, caption: str,
    ...
    if path.stat().st_size >= TG_LIMIT:
        ...
        if LEECH_URL:
            ...
            # post the resulting download link to the chat.
            try:
                import aiohttp
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(f"{LEECH_URL}/upload", data=data,
                                            timeout=aiohttp.ClientTimeout(total=7200)) as resp:
                        if resp.status == 200:
                            result = await resp.json()
                            dl_link = result.get("url") or result.get("link") or result.get("download_url", "")
                        else:
                            dl_link = ""
                if dl_link:
                    msg = (f"🔗 **[{label}]** `{size_gb:.2f} GB` — too large for Telegram\n"
                           f"**Direct Download:** {dl_link}")
                    try: await prog_msg.edit(msg)
                    except: pass
                    return
                else:
                    log.warning(f"[{label}] leech server returned no link, falling back to local HTTP")
            except Exception as e:
                log.warning(f"[{label}] leech upload failed: {e}, falling back to local HTTP")

        # Fallback: serve from the local Lightning HTTP server
        _ensure_http_server()
        ...
        if pub:
            ...
            msg = (f"✅ **[{label}]** encoded (`{size_gb:.2f} GB`). "
                   f"Expose port `{HTTP_PORT}` and access `{dl}`")
        try: await prog_msg.edit(msg)
        except: pass
        return
...
async def start_stream_server():
    ...
    stream_port = int(os.getenv("STREAM_PORT", "8181"))
    await _web.TCPSite(_stream_runner, stream_host, stream_port).start()
    log.info(f"Telegram stream server on {stream_host}:{stream_port}")
...
async def main():
    ...
    log.info(f"GPUs: {NUM_GPUS} | Encode semaphore: {ENCODE_CONCURRENCY} concurrent | Full-frame OCR")
    await app.start()
